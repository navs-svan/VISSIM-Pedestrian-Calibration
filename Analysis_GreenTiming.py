...
if __name__ == '__main__':
    ...
    set_parameters(Vissim, walking_behavior_param['CW1'])  # CHANGE
    time = (90 * 40) + 3
    ...
    for volume in range(5000, 5250, 250):
        if volume == 0:
            volume = 50
        print(volume)
        # Vissim.Net.PedestrianInputs.ItemByKey(sm_aura_id).SetAttValue('Volume(1)', volume)
        Vissim.Net.PedestrianInputs.ItemByKey(sm_aura_id).SetAttValue('Volume(1)', 0)
        ...
        curr_ped_number_1 = 0
        curr_ped_number_2 = 0
        previous_ped_number_1 = 0
        previous_ped_number_2 = 0
        previous_state = 'RED'
        counter = 0
        for i in range((time * int(Vissim.Simulation.AttValue('SimRes')))):
            ...
            if sig_state == 'RED':
                All_Ped = Vissim.Net.Pedestrians.GetMultipleAttributes(('No', 'ConstrElType', 'SimSec', 'Speed'))
                for cur_ped in All_Ped:
                    ped_number = cur_ped[0]
                    ped_location = cur_ped[1]
                    ped_sim_sec = cur_ped[2]
                    ped_speed = cur_ped[3]
                    if ped_location == 'PEDESTRIANLINK' \
                            and current_green_start <= ped_sim_sec <= (current_green_start + 45):
                        additional_time.add(ped_sim_sec)
            elif sig_state == 'GREEN' and previous_state == 'RED':
                curr_ped_number_1 = Vissim.Net.PedestrianTravelTimeMeasurements.ItemByKey(1).AttValue(
                    'Peds(Current,Current,40)')
                curr_ped_number_2 = Vissim.Net.PedestrianTravelTimeMeasurements.ItemByKey(2).AttValue(
                    'Peds(Current,Current,40)')
                ...
                ped_number_1 = curr_ped_number_1 - previous_ped_number_1
                # ped_number_2 = curr_ped_number_2 - previous_ped_number_2
                # number_summary.append((ped_number_1, ped_number_2))
                number_summary.append(ped_number_1)

                additional_time.clear()
                previous_ped_number_1 = curr_ped_number_1
                # previous_ped_number_2 = curr_ped_number_2
            if i == (time * int(Vissim.Simulation.AttValue('SimRes'))) - 1:
                speed = Vissim.Net.AreaMeasurements.ItemByKey(1).AttValue('SpeedAvg(Current,Current,40)')
        ...
        # print(speed_summary_global)
        print(speed)
        Vissim.Simulation.Stop()

    with open('clearance2.csv', 'w') as csvfile:
        csvwriter = csv.writer(csvfile, delimiter=',')
        csvwriter.writerows(clearance_summary_global)

    with open('number2.csv', 'w') as csvfile:
        csvwriter = csv.writer(csvfile, delimiter=',')
        csvwriter.writerows(number_summary_global)

    with open('speed2.csv', 'w') as csvfile:
        csvwriter = csv.writer(csvfile, delimiter=',')
        csvwriter.writerow(speed_summary_global)
    #
    # for line in clearance_summary_global:
    #     print(line)
